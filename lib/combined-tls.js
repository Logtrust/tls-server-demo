...
const opts = stdio.getopt({
  ...
  'new': {key: 'n', description: 'Use new TLSSocket()', default: false},
  'conn': {key: 'c', args: 1, description: 'Number of connections', default: 2000},
})
...
function loadtest() {
  if (cluster.isMaster) {
    console.log('Using %s event on %s', opts.readable ? 'readable' : 'data',
      opts.new ? 'new TLSSocket()' : 'createSecurePair()')
    showEvery()
    createServer(() => {
      createProxy(() => {
