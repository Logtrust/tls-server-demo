...
const packet = Buffer.from('xzvzcvz4239472398472383294dasfad'.repeat(100))
...
const opts = stdio.getopt({
  'readable': {key: 'r', description: 'Enable LZ4 compression', default: false},
    ...
    if (opts.readable) {
      first.on('readable', () => {
        ...
        if (!second.write(data)) {
          this.paused = true
          second.once('drain', () => this.paused = false)
        }
      })
    } else {
      first.on('data', data => {
