...
function getTraffic(bytes, elapsed) {
  const mb = bytes / 1e6
  const rate = mb / (elapsed / 1000)
  return util.format("%s MB (%s MB/s)", mb.toFixed(0), rate.toFixed(0))
}
...
function showEvery() {
  ...
  setInterval(() => {
    ...
    console.log('Elapsed %s s, proxied %s, %s events (%s bytes/event)',
      seconds.toFixed(0), getTraffic(proxyPipe.piped, elapsed),
      proxyPipe.events, rate.toFixed(0))
  }, showInterval).unref()
}
